...
class DrawingClassifier:
    ...
    def classes_prompt(self):
        ...
        self.proj_name = simpledialog.askstring("Proje İsmi", "Lütfen Aşağıya Proje İsminizi Giriniz", parent=msg)
        if os.path.exists(self.proj_name):
            ...
            self.class1 = simpledialog.askstring("Sınıf 1", "Birinci sınıfı ne olarak Çağıracaksınız?", parent=msg)
            self.class2 = simpledialog.askstring("Sınıf 2", "İkinci sınıfı ne olarak Çağıracaksınız?", parent=msg)
            self.class3 = simpledialog.askstring("Sınıf 3", "Üçüncü sınıfı ne olarak Çağıracaksınız?", parent=msg)
    ...
    def init_gui(self):
        ...
        self.root = Tk()
        self.root.title(f"Çizim Sınıflandırıcı - {self.proj_name}")
    ...
    def train_model(self):
        ...
        tkinter.messagebox.showinfo("Çizim Sınıflandırıcı", "Model Başarı İle Editlendi!", parent=self.root)
    ...
    def predict(self):
        ...
        if prediction[0] == 1:
            tkinter.messagebox.showinfo("NeuralNine Drawing Classifier", f"Çizim Büyük Olasılıkla {self.class1}", parent=self.root)
        elif prediction[0] == 2:
            tkinter.messagebox.showinfo("NeuralNine Drawing Classifier", f"Çizim Büyük Olasılıkla {self.class2}", parent=self.root)
        elif prediction[0] == 3:
            tkinter.messagebox.showinfo("NeuralNine Drawing Classifier", f"Çizim Büyük Olasılıkla {self.class3}", parent=self.root)
    ...
    def rotate_model(self):
        ...
        self.status_label.config(text=f"Geçerli Olan Model: {type(self.clf).__name__}")

    def save_model(self):
        file_path = filedialog.asksaveasfilename(defaultextension="pickle")
        with open(file_path, "wb") as f:
            pickle.dump(self.clf, f)
        tkinter.messagebox.showinfo("Çizim Sınıflandırıcı", "Model Başarıyla Kaydedildi!", parent=self.root)

    def load_model(self):
        file_path = filedialog.askopenfilename()
        with open(file_path, "rb") as f:
            self.clf = pickle.load(f)
        tkinter.messagebox.showinfo("Çizim Sınıflandırıcı", "Model Başarıyla Yüklendi", parent=self.root)
    ...
    def save_everything(self):
        ...
        with open(f"{self.proj_name}/{self.proj_name}_data.pickle", "wb") as f:
            pickle.dump(data, f)
        tkinter.messagebox.showinfo("Çizim Sınıflandırıcı", "Proje Başarıyla Kaydedildi!", parent=self.root)

    def on_closing(self):
        answer = tkinter.messagebox.askyesnocancel("Çıkış?", "Çalışmanızı Kayetmek İster misin?", parent=self.root)
